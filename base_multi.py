...
class COST_Classifier:
    ...
    def process(self, arr, labels):
        ...
        for index, value in enumerate(arr):
            ...
            value_index_map[value]['labels'].extend(labels[index])
        ...
        for labels in y:
            for label in labels:
                if label in label_map:
                    ...
                    label_map[label] = 1
        classes = sorted(list(label_map.keys()))

        for col in range(X.shape[1]):
            value_index_map = self.process(X[:, col], y)
    ...
    def ComputeFisher(self, table, method):
        """get p value by fisher exact test's upper bound
    ...
    def checkR(self, X_train, y_train, X_test, y_test, target_list, method):
        ...
        for r in range(X_test.shape[1]):
            r_table = info[:, :, r].T
            r_table_p = stats.beta.cdf(r_table, r + 1, X_test.shape[1] - r)
            res_reject_refine = []
            for i, row in enumerate(r_table_p):
                significant = np.where(row < 0.05)[0]
                if len(significant) == 1:
                    res_reject_refine.append(significant[0])
                elif len(significant) > 1:
                    res_reject_refine.append(significant)
            ...
            if score_reject_refine > r_best_score:
                r_best = r
                r_best_score = score_reject_refine
        return r_best
    ...
    def feature_subset_generation(self, train_x, valid_x, test_x, train_y, b1, b2):
        ...
        for i in range(new_subset_num):
            ...
            best_index = self.transform(new_array_train.T[sample_indices, :], [train_y[i] for i in sample_indices])
